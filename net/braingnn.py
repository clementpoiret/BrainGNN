...
class Network(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, edge_attr, pos):
        ...
        return x, self.pool1.weight, self.pool2.weight, torch.sigmoid(
            score1).view(x.size(0),
                         -1), torch.sigmoid(score2).view(x.size(0), -1)
